...
submitButton?.addEventListener("click", (e: MouseEvent): void => {
  ...
  // Hiding all validation error messages and error styles
  document
    .querySelectorAll(".contact-form__input--invalidated")
    .forEach((invalidatedInput: Element): void => {
      invalidatedInput.classList.remove("contact-form__input--invalidated");
    });
  document
  ...
  inputs.forEach((input: HTMLInputElement): void => {
    if (input.value === "") {
      ...
      input.classList.add("contact-form__input--invalidated");
    }
  });
  ...
  if (
    ...
    emailInput?.classList.add("contact-form__input--invalidated");
  }
